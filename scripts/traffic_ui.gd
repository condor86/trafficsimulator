...
func _on_distance_cell_submitted(_text: String, col_idx: int) -> void:
	...
	_clamp_single_distance(col_idx)
...
# 只 clamp 当前列，不动其它列
func _clamp_single_distance(col_idx: int) -> void:
	...
	if col_idx == 1:
		var lower := a + DIST_MIN_GAP
		var upper := DIST_MAX_D
		if c > 0.0:
			upper = minf(c - DIST_MIN_GAP, DIST_MAX_D)
		if upper < lower:
			upper = lower
		b = clampf(b, lower, upper)
		(dist_row[1] as LineEdit).text = str(roundi(b))
	elif col_idx == 2:
		var lower := b + DIST_MIN_GAP
		var upper := DIST_MAX_D
		if d > 0.0:
			upper = minf(d - DIST_MIN_GAP, DIST_MAX_D)
		if upper < lower:
			upper = lower
		c = clampf(c, lower, upper)
		(dist_row[2] as LineEdit).text = str(roundi(c))
	elif col_idx == 3:
		var lower := c + DIST_MIN_GAP
		var upper := DIST_MAX_D
		if upper < lower:
			upper = lower
		d = clampf(d, lower, upper)
...
func set_distances_from_root(dists: Array) -> void:
	...
	_apply_distance_clamp_and_write()
	_updating_distances = false
...
# 这个给 root 初始注入时用的“全行整理版”
func _apply_distance_clamp_and_write() -> void:
	if _light_fields.size() <= DIST_ROW_IDX:
		return
	var dist_row = _light_fields[DIST_ROW_IDX]
